...
- [ ] **Slice 1 — Authentication (GitHub OAuth2)**
  `User` + `UserIdentity` tables, Spring Security OAuth2 login (GitHub first), protected endpoint pattern established. Done early (moved up from the original draft) specifically so `UserVocabulary` in Slice 5 can reference a real logged-in user from the start, instead of a throwaway hardcoded test-user id that would need to be ripped out later. This doesn't mean every endpoint requires login, though: the reading/translation pipeline (Slices 3-4) must stay reachable by unauthenticated guests (see decisions.md section 35) — only vocabulary tracking (Slice 5) actually needs a real user. Avatar is pulled from the OAuth2 provider where possible, with click-to-logout in the UI; the login entry point must also be reachable from the guest-facing Text page, not just a dedicated login screen.
